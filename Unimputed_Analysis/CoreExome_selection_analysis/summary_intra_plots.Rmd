...
lsa.gwas.cluster <- function (gwas_data,settings) {
  ...
  lambda_cluster <- function (position, eps = 0.2, minPts = 1) {
...
plotTracks(list(DataTrack(cau_ihs_regions[seqnames(cau_ihs_regions) == i], data = abs(cau_ihs_regions[seqnames(cau_ihs_regions) == i]$std_ihs), name = "nzcau ihs", col = 'red'),
                cau = AnnotationTrack(cau_regions[seqnames(cau_regions) == i & width(cau_regions) >= 1], name = "nzcau key peaks"),
                DataTrack(nzm_ihs_regions[seqnames(nzm_ihs_regions) == i], data = abs(nzm_ihs_regions[seqnames(nzm_ihs_regions) ==i]$std_ihs), name = "nzm ihs", col = 'blue'),
                nzm = AnnotationTrack(nzm_regions[seqnames(nzm_regions) == i & width(nzm_regions) > 1]),
                DataTrack(cim_ihs_regions[seqnames(cim_ihs_regions) == i], data = abs(cim_ihs_regions[seqnames(cim_ihs_regions) == i]$std_ihs), name = 'cim ihs', col = 'orange'),
                cim = AnnotationTrack(cim_regions[seqnames(cim_regions)== i & width(cim_regions) > 1]),
                DataTrack(ton_ihs_regions[seqnames(ton_ihs_regions) == i], data = abs(ton_ihs_regions[seqnames(ton_ihs_regions) == i]$std_ihs), name = 'ton ihs', col='green'),
                ton = AnnotationTrack(ton_regions[seqnames(ton_regions)== i & width(ton_regions) > 1]),
                DataTrack(sam_ihs_regions[seqnames(sam_ihs_regions) == i], data = abs(sam_ihs_regions[seqnames(sam_ihs_regions) == i]$std_ihs), name = 'sam ihs', col = 'purple'),
...
library(GenomicRanges)
cau_nsl <- combined_nsl %>% filter(POP == "nzcau" & abs(std_nsl) > 3)
nzm_nsl <- combined_nsl %>% filter(POP == "nzm" & abs(std_nsl) > 3)
cim_nsl <-combined_nsl %>% filter(POP == "cim" & abs(std_nsl) > 3)
ton_nsl <- combined_nsl %>% filter(POP == "ton" & abs(std_nsl) > 3)
sam_nsl <- combined_nsl %>% filter(POP == "sam" & abs(std_nsl) > 3)
...
for(i in 1:22){
  print(paste("Chromosome", i))
plotTracks(list(DataTrack(cau_nsl_regions[seqnames(cau_nsl_regions) == i], data = abs(cau_nsl_regions[seqnames(cau_nsl_regions) == i]$std_nsl), name = "nzcau nsl", col = 'red'),
                cau = AnnotationTrack(cau_regions[seqnames(cau_regions) == i & width(cau_regions) >= 1], name = "nzcau key peaks"),
                DataTrack(nzm_nsl_regions[seqnames(nzm_nsl_regions) == i], data = abs(nzm_nsl_regions[seqnames(nzm_nsl_regions) ==i]$std_nsl), name = "nzm nsl", col = 'blue'),
                nzm = AnnotationTrack(nzm_regions[seqnames(nzm_regions) == i & width(nzm_regions) > 1]),
                DataTrack(cim_nsl_regions[seqnames(cim_nsl_regions) == i], data = abs(cim_nsl_regions[seqnames(cim_nsl_regions) == i]$std_nsl), name = 'cim nsl', col = 'orange'),
                cim = AnnotationTrack(cim_regions[seqnames(cim_regions)== i & width(cim_regions) > 1]),
                DataTrack(ton_nsl_regions[seqnames(ton_nsl_regions) == i], data = abs(ton_nsl_regions[seqnames(ton_nsl_regions) == i]$std_nsl), name = 'ton nsl', col='green'),
                ton = AnnotationTrack(ton_regions[seqnames(ton_regions)== i & width(ton_regions) > 1]),
                DataTrack(sam_nsl_regions[seqnames(sam_nsl_regions) == i], data = abs(sam_nsl_regions[seqnames(sam_nsl_regions) == i]$std_nsl), name = 'sam nsl', col = 'purple'),
                sam = AnnotationTrack(sam_regions[seqnames(sam_regions)== i & width(sam_regions) > 1]),
                GenomeAxisTrack()
                )
           )
...
#
#apply(as.data.frame(unique_nzcau_regions[['faw']]),1, function(x){queryUCSC(fromUCSCEnsemblGenes(chromosome = x[1], start = x[2], end = x[3]))})
...
target <- GRanges(paste0('chr',seqnames(unique_nzm_regions[['faw']])), IRanges(unique_nzm_regions[['faw']]@ranges))
#annotate variants
...
n <- merge(as.data.frame(loc), symbols, by.x = "GENEID", by.y = "ENTREZID", all.x=TRUE)
