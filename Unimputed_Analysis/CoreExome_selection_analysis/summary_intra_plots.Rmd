...
get_nsl <- function(nsl_path, pop){
  nsl <- list()
  for(i in 1:22){
    a= read.table(paste0(nsl_path,pop,'_',i,'.nsl.out.100bins.norm'), header=TRUE)
...
get_ihs <- function(ihs_path, pop){
  ihs <- list()
  for(i in 1:22){
    a= read.table(paste0(ihs_path,pop,'_',i,'.ihs.out.100bins.norm'), header=TRUE)
...
lsa.gwas.cluster<-function (gwas_data) {
  # Cluster GWAS data to identify regions of association
  # https://github.com/MerrimanLab/ResequencingAnalysis/blob/master/LociSelectionApp/lsa_logic.R
  lambda_cluster<-function (position,eps=0.2,minPts=1) {
    dbscan::dbscan(as.matrix(position)/1000000,eps=eps,minPts= minPts)
  }
  gwas_data[,ClusterID:= lambda_cluster(position),by= c("chrom")]
  return (gwas_data)
}
...
calc_dist_mat <- function(popList){
...
```{r load data}
nzcau <-list(faw = get_faw(paste0(data_dir,"faywuh/"),"nzcau"), td = get_td(paste0(data_dir, "tajimasd/"), 'nzcau'), ihs = get_ihs(paste0(data_dir,"ihs/nzcau/"), 'nzcau'), nsl = get_nsl(paste0(data_dir,"nsl/nzcau/"), 'nzcau'))
nzm <-list(faw = get_faw(paste0(data_dir,"faywuh/"),"nzm"), td = get_td(paste0(data_dir, "tajimasd/"), 'nzm'), ihs = get_ihs(paste0(data_dir,"ihs/nzm/"), 'nzm'), nsl = get_nsl(paste0(data_dir,"nsl/nzm/"), 'nzm'))
cim <-list(faw = get_faw(paste0(data_dir,"faywuh/"),"cim"), td = get_td(paste0(data_dir, "tajimasd/"), 'cim'), ihs = get_ihs(paste0(data_dir,"ihs/cim/"), 'cim'), nsl = get_nsl(paste0(data_dir,"nsl/cim/"), 'cim'))
ton <-list(faw = get_faw(paste0(data_dir,"faywuh/"),"ton"), td = get_td(paste0(data_dir, "tajimasd/"), 'ton'), ihs = get_ihs(paste0(data_dir,"ihs/ton/"), 'ton'), nsl = get_nsl(paste0(data_dir,"nsl/ton/"), 'ton'))
sam <-list(faw = get_faw(paste0(data_dir,"faywuh/"),"sam"), td = get_td(paste0(data_dir, "tajimasd/"), 'sam'), ihs = get_ihs(paste0(data_dir,"ihs/sam/"), 'sam'), nsl = get_nsl(paste0(data_dir,"nsl/sam/"), 'sam'))
```
